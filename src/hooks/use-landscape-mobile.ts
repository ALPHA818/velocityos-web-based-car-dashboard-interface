...
function computeIsLandscapeMobile(): boolean {
  ...
  return isLandscape && isMobileLike
}

export function useIsLandscapeMobile() {
  const [isLandscapeMobile, setIsLandscapeMobile] = React.useState(computeIsLandscapeMobile)

  React.useEffect(() => {
    if (typeof window === "undefined") return

    const update = () => {
      setIsLandscapeMobile(computeIsLandscapeMobile())
    }
