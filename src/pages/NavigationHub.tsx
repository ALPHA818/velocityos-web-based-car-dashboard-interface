...
import { MapPin, Home, Briefcase, Star, Plus, Compass, Clock, Trash2 } from 'lucide-react';
...
export function NavigationHub() {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    if (isNaN(lat) || lat < -90 || lat > 90) return toast.error('Invalid Latitude');
    if (isNaN(lon) || lon < -180 || lon > 180) return toast.error('Invalid Longitude');
    try {
      await addLocation({
        ...
        items.map((loc, idx) => {
          ...
          return (
            <motion.button
              ...
              onClick={() => openMap(loc)}
              className="dashboard-card flex flex-col items-start text-left gap-4 hover:border-primary/50 group bg-zinc-900/40"
            >
              <div 
                ...
                <div className="flex justify-between items-start">
                  ...
                  {loc.lastUsedAt && (
                    <span className="text-xs text-muted-foreground whitespace-nowrap mt-2">
                      {formatDistanceToNow(loc.lastUsedAt)} ago
                    </span>
                  )}
                </div>
      ...
      <div className="max-w-7xl mx-auto space-y-10">
        <header className="flex justify-between items-end">
          <div className="space-y-2">
            ...
            <p className="text-xl text-muted-foreground font-medium">Where are we going today?</p>
          </div>
          <div className="flex gap-4">
             <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
              <DialogTrigger asChild>
                <Button size="lg" className="rounded-[2rem] h-20 px-10 gap-3 bg-primary text-primary-foreground shadow-glow-lg text-xl font-bold">
              ...
              <DialogContent className="bg-zinc-950 border-white/10 text-white sm:max-w-[550px] rounded-[3rem] p-10">
                ...
                <form onSubmit={handleSave} className="space-y-8 pt-6">
                  <div className="space-y-3">
                    ...
                    <Input value={formData.label} onChange={e => setFormData(p => ({...p, label: e.target.value}))} className="h-16 bg-zinc-900 border-white/5 rounded-2xl text-xl" />
                  </div>
                  <div className="grid grid-cols-2 gap-6">
                    <div className="space-y-3">
                      <Label className="text-lg">Latitude</Label>
                      <Input value={formData.lat} onChange={e => setFormData(p => ({...p, lat: e.target.value}))} className="h-16 bg-zinc-900 border-white/5 rounded-2xl text-xl" />
                    </div>
                    <div className="space-y-3">
                      <Label className="text-lg">Longitude</Label>
                      <Input value={formData.lon} onChange={e => setFormData(p => ({...p, lon: e.target.value}))} className="h-16 bg-zinc-900 border-white/5 rounded-2xl text-xl" />
                    </div>
                  </div>
                  <div className="space-y-3">
        ...
        <Tabs defaultValue="favorites" className="space-y-8">
          ...
          <TabsContent value="favorites">
            <LocationGrid items={locations} emptyMsg="No favorites saved yet" />
          </TabsContent>
          <TabsContent value="recents">
            <div className="space-y-6">
              <div className="flex justify-end">
                <Button variant="ghost" className="text-muted-foreground hover:text-destructive gap-2 h-14 rounded-2xl px-6" onClick={clearHistory}>
